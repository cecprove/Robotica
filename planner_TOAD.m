...
pf=u(1:2); %punto finale per il primo tratto, coincide con il punto B
p_i=u(3:4); %punto iniziale per il primo tratto, coincide con il punto C
...
phi_f=u(8); %phi iniziale per il primo tratto, ovvero in C
phi_i=u(9); %phi finale per il primo tratto, ovvero in B
alfa_2=deg2rad(270); %angolo di rotazione per il primo tratto
if t<=tf %se ci troviamo ad un tempo inferiore di tf, dobbiamo pianificare
    %la traiettoria circolare del primo tratto, con raggio 'r2' e centro
    %'c2'
A_2=[ti^3 ti^2 ti 1;
   tf^3 tf^2 tf 1;
   3*ti^2 2*ti 1 0;
...
B_2=[0;r2*alfa_2;0;0]; %vettore dei termini noti per la traiettoria
...
R=[cos(pi/4) sin(pi/4);
    -sin(pi/4) cos(pi/4)];%rotazione per la pianificazione di una traiettoria
%circolare, sappiamo che l'equazione si riferisce ad una terna concentrica
%a 'c2' e con angolo nullo quando ci troviamo nella situazione s(0)
...
if t<ti %se ci troviamo ad un tempo inferiore a quello iniziale allora ci troviamo nella
    ...
    XYd=(c2'+R*[r2*cos(s/r2) r2*sin(s/r2)]')'; %è la x desiderata
...
B_1=[0;r1*pi;0;0]; %vettore termini noti per la traiettoria
...
R1=[cos(pi) sin(pi);
    -sin(pi) cos(pi)]; %matrice di rotazione per rimetterci nelle condizioni
%per la pianificazione della traiettoria con centro 'c1' e raggio 'r1'
...
sdot_phi=3*a_1phi(1)*t^2+2*a_1phi(2)*t+a_1phi(3); %derivata legge oraria orientamento
    ...
    XYd=(c1'+R1*[r1*cos(s/r1) r1*sin(s/r1)]')'; %x desiderata
    XYddot=(R1*[-sin(s/r1) cos(s/r1)]')';

    phi_e=phi_f+(s_phi/norm(phi_f-phi_i))*(phi_i-phi_f); %phi desiderata
    phi_ed=(sdot_phi/norm(phi_f-phi_i))*(phi_i-phi_f);
    else %altrimenti rimaniamo alla configurazione da cui siamo partiti ovvero nel punto C
    XYd=p_i;
    XYddot=[0;0];
    phi_e=phi_i;
    phi_et=[0;0];
